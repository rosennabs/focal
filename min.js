/* PSEUDOCODE:
First assign the first element of the array to smallestNumber.
Then create a for loop that will run through the rest of the elements in the array.

  if (smallestNumber < the next element) {
    keep smallestNumber
  } else if (smallestNumber > the next element) {
    assign element to smallestNumber
  }
  Then return smallestNumber
*/

const min = function(numbers) {
  let smallestNumber = numbers[0];//assigns the first element of the array to smallestNumber
  for (let i = 1; i < numbers.length; i++) {//loops through the remaining elements in the array
